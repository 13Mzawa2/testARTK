...
static int init(int argc, char *argv[])
{
 ...
 //   	return (FALSE);
	...
	cv::Mat temp;
	kinect.getColorFrame(temp);
	safeRelease(kinect.colorFrame);
	gXsize = temp.cols; gYsize = temp.rows;
...
static void grabImage(void) {
	...
	do {
		kinect.getColorFrame(temp);
		safeRelease(kinect.colorFrame);
		cv::cvtColor(temp, temp2, CV_BGR2BGRA);
		image = (ARUint8*)temp2.data;
...
static void Idle(void)
{
	...
	if (gStatus == 0) {
		...
		safeRelease(kinect.colorFrame);
		cv::cvtColor(temp, temp2, CV_BGR2BGRA);
		ARUint8 *image = (ARUint8*)temp2.data;
		if (image != NULL) {
			gARTImage = image;
			// Tell GLUT the display has changed.
			glutPostRedisplay();
...
static void Display(void)
{
    ...
    if (gStatus == 0) {
		cv::Mat temp(cv::Size(gXsize, gYsize), CV_8UC4);
		::memcpy(temp.data, gARTImage, temp.total()*temp.channels());
		//cv::imshow("test", temp);
		//cv::waitKey(0);
